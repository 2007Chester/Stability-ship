...
with tab_hold_view:
    ...
    st.caption(
        ...
        f"**Выше главной палубы** — борт комингсов люка (**~{COAMING_HEIGHT_ABOVE_DECK_M} м** от палубы вверх — правьте в `hold_layout.py` по GA). "
        "**Грузовой трюм** — **под главной палубой** до внутреннего дна. "
        f"По длине от кормы: **{HOLD_LCG_AP_AFT_M:.0f}…{HOLD_LCG_AP_FWD_M:.0f} м**; вн. дно над килем **{INNER_BOTTOM_ABOVE_KEEL_M} м**. "
        f"Шаг шпангоутов: **{FRAME_SPACING_MAIN_M} м**; **{NUM_HOLD_SECTIONS}** секций по длине трюма."
    )
    ...
    cargo_below = float(stow_d["cargo_surface_below_main_deck_m"])
    h_mould = float(stow_d["hold_depth_moulded_m"])
    h_coom_up = float(stow_d["coaming_height_above_main_deck_m"])
    st.info(
        f"**Равномерно по {NUM_HOLD_SECTIONS} секциям:** ~**{t_per_sec:.1f} т** на секцию. "
        f"**Выше главной палубы:** комингсы **{h_coom_up:.2f} м** (**от палубы вверх** до верха комингсов). "
        f"**Под палубой** — глубина трюма **{h_mould:.2f} м**; поверхность груза **ниже главной палубы на {cargo_below:.2f} м**; "
        f"насыпь от внутреннего дна **{h_from_tt:.2f} м**. "
        f"**От поверхности груза вверх** до верха комингсов: **{clr_coom:.2f} м**. Объём **≈ {vol_m3:,.0f} м³**. **KG** — «Остойчивость»."
    )
    ...
    with st.expander("Как читать схему"):
        st.markdown(
            f"""
- **Масштаб:** по осям **1:1** (1 м по длине = 1 м по высоте) — корпус визуально «низкий», это нормально при L/D ≈ **{LBP_M / float(SHIP['depth_m']):.0f}**.
- **Ось X** — от **шп. кормы**, **Y** — от **киля** вверх (как на GA).
- **Корпус** — **LBP × D**; **главная палуба** — молдинг **D**; **выше палубы** — **комингсы** люка (**от палубы вверх**); **двойное дно** под трюмом.
- **Грузовой трюм** — **под главной палубой** до внутреннего дна (глубина **≈ {hold_moulded_depth_m():.2f} м**); по длине **{HOLD_LCG_AP_AFT_M:.0f}–{HOLD_LCG_AP_FWD_M:.0f} м**; **20 секций** (~**{hold_length_m() / NUM_HOLD_SECTIONS:.2f} м**); **1** у кормы, **20** у носа.
- **Уголь** — насыпь от внутреннего дна; пунктир — **поверхность груза**; **ниже палубы на … м** — до поверхности груза; **вверх от груза** — зазор до **верха комингсов** (выше палубы).
            """
        )
