...
with tab_hold_view:
    ...
    st.caption(
        ...
        "**Грузовой трюм** — объём **ниже главной палубы** (сверху палуба, снизу внутреннее дно). "
        f"По длине от кормы: **{HOLD_LCG_AP_AFT_M:.0f}…{HOLD_LCG_AP_FWD_M:.0f} м**; вн. дно над килем **{INNER_BOTTOM_ABOVE_KEEL_M} м**, комингсы **~{COAMING_HEIGHT_ABOVE_DECK_M} м** над палубой (уточняйте в `hold_layout.py` по GA). "
    ...
    d_deck = float(stow_d["depth_from_main_deck_to_cargo_m"])
    h_mould = float(stow_d["hold_depth_moulded_m"])
    st.info(
        f"**Равномерно по {NUM_HOLD_SECTIONS} секциям:** ~**{t_per_sec:.1f} т** на секцию. "
        f"**Глубина трюма** (главная палуба → внутреннее дно): **{h_mould:.2f} м**. "
        f"**От главной палубы вниз до поверхности груза:** **{d_deck:.2f} м**; "
        f"**насыпь от внутреннего дна:** **{h_from_tt:.2f} м**. "
        f"**Зазор до верха комингсов:** **{clr_coom:.2f} м**. Объём груза **≈ {vol_m3:,.0f} м³**. **KG** — вкладка «Остойчивость»."
    )
...
- **Корпус** — **LBP × D**; **двойное дно** — ниже внутреннего дна трюма; **главная палуба** — уровень молдинга **D**; **комингсы** — над палубой (люк).
- **Грузовой трюм** — **от главной палубы вниз** до внутреннего дна (глубина **≈ {hold_moulded_depth_m():.2f} м** при текущих константах); по длине **{HOLD_LCG_AP_AFT_M:.0f}–{HOLD_LCG_AP_FWD_M:.0f} м**; **20 секций** (~**{hold_length_m() / NUM_HOLD_SECTIONS:.2f} м**); **1** у кормы, **20** у носа.
- **Уголь** — насыпь от внутреннего дна; пунктир — **поверхность груза**; **«от палубы вниз»** — расстояние от главной палубы до этой поверхности; зазор — до **верха комингсов**.
            """
        )
